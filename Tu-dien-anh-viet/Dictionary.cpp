...
void Dictionary::search() {
    ...
    // Tìm kiếm, nhấn phím esc hoặc phím mũi tên để thoát 
    wstring eng;
    while (true)
    {
        gotoxy(4, 1); wcout << eng;
        char ch = _getch();
        fflush(stdin);
           
        if (ch == -32) // Nếu bấm phím mũi tên bất kì
        {
            if (_getch() == 75) // Phím mũi tên trái
            {
                break;
            }
        }
        ...
        if (ch == 8) { // backspace
            if (eng.length() > 0) {
                gotoxy(4 + eng.length() - 1, 1);
                wcout << " ";
                eng.pop_back();

            }
        }
        else if (('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z') || ch == 32) // Nếu bấm chữ cái hoặc phím cách
        {
            eng.push_back(ch);
        }
        ...
        size_t count = 0;
        for (unsigned int i = 0; eng.length() != 0 &&
                                i < vectorWordSorted.size() && count < 27; i++) {
            if (vectorWordSorted[i].find(eng) == 0) {
                gotoxy(4, 2 + count);
        ...
        //if (exists)
        //{
        //    wcout << std::left << setw(85) << this->dataDictionary.at(eng);
        //}
        //else {
        //    wcout << setw(85) << "=)";
        //}
